...
`validation-strategy` capability 是项目的方法学治理层：冻结 validity
taxonomy（4 类接受 / 1 类拒绝 / 1 类 stub）+ stereotype audit 三协议
（swap / blind / cross-model）+ face validity protocol（Prolific N=20 ×
M=10 narrative）+ population & behavioral calibration target（ABS Census
2021 + ABS Travel Survey + Google Popular Times）+ reproducibility lock
七字段 + pre-publication checklist 8 项。

本 capability SHALL 是**纯 spec capability**——不引入任何代码模块（不创建
`synthetic_socio_wind_tunnel/validation/` 目录）；它的作用是约束所有
publishable artifact（report.md / contest.json / poster / paper draft）
在产出前通过 8 项 checklist，并为下游 change（`agent-calibration` /
`stereotype-audit` / `population-realdata` / `behavioral-calibration` /
`face-validity-protocol` 等）提供 acceptance 阈值与执行协议的引用源。

本 spec 与 `experimental-design`（实验语法 / 措辞门禁）/ `metrics`
（统计实现 / contest 打分）/ `suite-wiring`（CLI suite 编排）等已有
spec 在职责上互补：experimental-design 管 "怎么写实验"，metrics 管
"怎么算数字"，suite-wiring 管 "怎么跑 suite"，validation-strategy 管
"什么样的数字才能发布"。
