"""Extract 36F Plaza cafe owner (a_44_0059) from all 4 seed-44 snapshots.

Adapted from extract_4variants.py (Mary). Output to data/analysis/case_studies/barista_4variants.json.
"""
import ijson
import json
...
OUT = REPO / "data/analysis/case_studies/barista_4variants.json"
...
print(f"Extracting {HERO} (36F Plaza cafe owner, seed 44) from 4 variants...", flush=True)
...
for v in ["baseline", "hyperlocal_push", "global_distraction", "phone_friction"]:
    ...
    print(f"    dialogue infos: {len(d.get('dialogue_infos', []))}")
