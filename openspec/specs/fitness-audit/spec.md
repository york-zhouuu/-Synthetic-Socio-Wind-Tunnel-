...
用一组可执行的 acceptance test + 诊断脚本，逐条验证 Phase 1 基建能否承载
三个实验（Digital Lure / Spatial Unlock / Shared Perception）。产出结构化
`fitness-report.json`，作为 Phase 2 每块 change 的"为什么要做"证据锚点。

审计分三组 category：
- **基线存在性**（`phase1-baseline` / `phase2-gaps`）：探针式检查能力是否存在。
- **集成**（`e1-* / e2-* / e3-* / profile-* / ledger-*`）：假设能力存在时测端到端。
- **诊断**（`site-fitness / scale-baseline / cost-baseline`）：纯数据快照。

模块：`synthetic_socio_wind_tunnel/fitness/`
...
- **THEN** 返回的 `FitnessReport` SHALL 覆盖全部 10 个 category，
  每条 `AuditResult` SHALL 具备非空 `id` 与 `status`

#### Scenario: 失败条目带修复指向
- **WHEN** `phase2-gaps.memory` 因模块不存在而失败
- **THEN** 该 AuditResult 的 `mitigation_change` SHALL 非空（指向
  `"memory"` 等 Phase 2 change 名）
...
- **scale-baseline**：`scale="quick"` 下 100 agent × 72 tick 的 wall 时间 p50/p99，
  每 tick 执行 move_entity + pipeline.render 的真实负载（不含 LLM）；
  `scale="full"` 下 1000 agent × 288 tick 的同样指标（可选跑）。
- **cost-baseline**：不实际调用 LLM，仅**估算**——按假定的 "每 agent 每天
  N 次 sonnet 调用 / M 次 haiku" 的 token 用量，乘以常量价格，输出日成本
  上下界。token / 价格常量为未经测量的占位，代码注释明标。

#### Scenario: quick 规模基线在 CI 下可跑
- **WHEN** CI 环境调用 `run_audit(scale="quick")`
- **THEN** 结果 SHALL 在单机 120 秒内返回；`scale_baseline.wall_seconds_p99`
  SHALL 非空
