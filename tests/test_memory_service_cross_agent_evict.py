...
def _add(service: MemoryService, agent_id: str, kind: str, tick: int) -> None:
    """Append an event into agent's store."""
    # Get or create store for agent
    ...
    service._stores[agent_id].append(MemoryEvent(
        event_id=f"ev_{agent_id}_{kind}_{tick}",
        agent_id=agent_id,
        tick=tick,
...
def test_cross_agent_evict_accumulates_count() -> None:
    """spec scenario: 跨 agent evict 累计."""
    service = MemoryService()
    for aid in ("a_001", "a_002", "a_003"):
        _add(service, aid, "encounter", 10)
        _add(service, aid, "encounter", 200)
    # 3 agents × 1 evicted each (tick=10 < 100) = 3
    n = service.evict_cold_encounter_events_across_agents(before_tick=100)
    assert n == 3
    for aid in ("a_001", "a_002", "a_003"):
        remaining = service._stores[aid].all()
        assert len(remaining) == 1
        assert remaining[0].tick == 200


def test_cross_agent_evict_idempotent() -> None:
    """spec scenario: 二次调用 idempotent."""
    service = MemoryService()
    _add(service, "a_001", "encounter", 10)
    _add(service, "a_001", "encounter", 200)
    first = service.evict_cold_encounter_events_across_agents(before_tick=100)
    second = service.evict_cold_encounter_events_across_agents(before_tick=100)
    assert first == 1
    assert second == 0
...
def test_cross_agent_evict_empty_service() -> None:
    ...
    service = MemoryService()
    n = service.evict_cold_encounter_events_across_agents(before_tick=999)
    assert n == 0
...
def test_cross_agent_evict_non_encounter_untouched() -> None:
    ...
    service = MemoryService()
    _add(service, "a_001", "action", 10)
    _add(service, "a_001", "encounter", 10)
    _add(service, "a_002", "reflection", 10)
    n = service.evict_cold_encounter_events_across_agents(before_tick=100)
    assert n == 1
