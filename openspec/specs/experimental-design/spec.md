...
Baseline/Intervention/Post）、publishable 严谨度（β 级：4 seed + CI）、
...
任何声明 "in-sim effect size" 的 publishable 数值 SHALL 由**至少 4 个不同
seed** 的 run 聚合而成，并以 **median + IQR [25, 75] 或 95% CI** 报告。

> **历史注**：原 spec 要求 β=30；2026-05-17 务实下调到 β=10；2026-05-18 再
> 降到 β=4。背景：1000 agent × 14 day × 4 variant publishable 在 DeepSeek +
> Volces Doubao 配置下，单 seed × 14 day wall ≈ 22 hr。受限于单机 48 GB 内存
> 上限 + Doubao 单 key RPM/TPM 配额，最大并行 = 4 seed × 4 variant = 16
> worker → ~1 天 wall。β=4 CI 宽度约比 β=30 大 **2.7 倍**——研究者报告时
> 显式标注 CI 宽度 + "preliminary β=4" 声明。

#### Scenario: 单 run 数字不得作 publishable claim
- **WHEN** 报告或展示中出现一个 numerical effect size
- **THEN** 该数字 SHALL 伴随 seed count ≥ 4 的证据；否则 MUST 显式标注
...
- **WHEN** seed 数 < 4 但研究者仍希望引用数字
- **THEN** 报告 SHALL 以 "preliminary" section 呈现，并列出为何未达 4
  seed 的原因
...
- **THEN** 它 SHALL 使用同样 4 seeds × 14 days 协议；结果 SHALL 与正向
