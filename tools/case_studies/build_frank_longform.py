"""Build 老 Frank's longform NYT-style profile HTML.
...
Output: docs/case_studies/frank.html
"""
import json
import re
...
OUT = REPO / "docs/case_studies/frank.html"

FRANK = "a_43_0012"

# ─── Load all data ─────────────────────────────────────────────────────
print("Loading data...")
four = json.load(open(DIARY_DIR / "frank_4variants.json"))
positions = json.load(open(DIARY_DIR / "frank_4variants_positions.json"))
...
# 老 Frank's profile from population_cache
import os
profiles = {}
...
mary_profile = profiles[FRANK]

# 老 Frank in HP (canonical narrative variant)
...
<section class="open">
  ...
  <h1>一位 64 岁退休会计去了健身房</h1>
  <p class="subtitle">基于 14 天算法风洞与 1,000 人仿真数据:
  观察 4 种不同的手机推送,如何把同一个住了 32 年 Lane Cove 的老 Frank,
  分别留在家里、推去教堂、漂在街上、和拉到了健身房。</p>
</section>
"""


def section_open_scene():
    """First scene — Frank at Anglican Church, evening."""
    entity = hp["ledger_entity"]
    ai_town = hp["agent_runtime_state"]["ai_town"]
    plan_steps = hp["agent_runtime_state"]["plan"]["steps"]
    plan_dest = plan_steps[0].get("destination") if plan_steps else "shinnyo_australia"
    return f"""
<section class="chapter scene-open">
  <div class="scene-time">2026 年 5 月 5 日 · 星期一 · 傍晚 20:30 · Lane Cove · clear · evening</div>
  <p>Anglican Church of Australia Lane Cove 的门廊灯刚刚亮起来。日落 4 个钟头之后,教堂的草坪还散着白天晒过的热气。
  a_43_0012 站在那里。<strong>系统记录他在 5 月 5 日傍晚 20:30 抵达此地,从那以后没再离开。</strong></p>
  <p>有意思的是,他这一天的 <code>plan_text</code> 里写的目的地是 <code>shinnyo_australia</code> ——
  一座日本佛教冥想中心,在 Burns Bay Road 拐角。<strong>他打算去的不是教堂,是寺院。
  但他走着走着,落到了这里</strong>。<code>plan.reason</code> 字段里仍然写着
  "<em>被 hyperlocal 推送吸引</em>"。</p>
  <p>他的 <code>invite_accept_probability</code> 是 <strong>{ai_town.get('invite_accept_probability', 0.8):.1f}</strong>
  —— 系统记录他这 14 天里有 80% 的概率会答应陌生人的邀请。<br>
  在他自己 64 年的真实人生里,他大概从来没有听过任何人这样描述过他自己。</p>
</section>
"""
...
<section class="methodology">
  <h2>这不是采访</h2>
  <p>老 Frank 是 <strong>Synthetic Socio Wind Tunnel</strong> 这套仿真系统里
  1,000 个虚拟居民中编号 a_43_0012 的那位。他的 14 天发生在 4 个平行实验里:
...
def section_who():
    """他是谁 — life_history + identity_text contradiction + shared_memory background"""
...
""")
    ...
    occ = mary_profile.get("occupation", "")

    return f"""
<section class="chapter chapter-who">
  <h2>1 · 他是谁(取决于你问哪一份数据)</h2>
  <p>系统给 a_43_0012 取的代号是 agent_12。我们暂且叫他 <strong>"老 Frank"</strong>——
  这不是他在仿真里用过的名字,是这篇报道为他取的代号,
  方便把他从其他 999 个 <code>a_43_xxxx</code> 里挑出来。</p>

  <p>这就是他——<strong>系统给他的 profile.identity_text 这么写</strong>:</p>
  <div class="profile-quote">{identity}</div>

  <p>但<strong>系统同时给了他 20 条 life_history(仿真启动前他的人生回忆)</strong>,
  这 20 条事件细致地讲了完全不同的另一个人:</p>

  {"".join(cards)}

  <p>把两层数据摆在一起,会看到一件有趣的事:</p>
  <div class="identity-conflict">
    <div class="ic-row">
      <span class="ic-label">profile 说</span>
      <span class="ic-val">"<strong>{occ}</strong> · 3 年前买的房 · <strong>内敛少言</strong>"</span>
    </div>
    <div class="ic-row">
      <span class="ic-label">life_history 说</span>
      <span class="ic-val"><strong>30 年会计</strong>(St Leonards 事务所,2015 退休,
        同事送他刻着"<em>Numbers Never Lie</em>"的笔筒) ·
        <strong>1992 年 3 月搬进 building_1712</strong>(32 年了,不是 3 年) ·
        每天 7:30 准时坐 wild_bean_cafe 角落小圆桌喝 flat white</span>
    </div>
  </div>

  <p>这两份数据是仿真系统用两个不同的 LLM prompt 在不同的时刻生成的,
  互相之间没有 cross-check。他自己当然不知道这两份数据互相打架。
  <strong>但在接下来 14 天里,他会反复 evangelize 一件事——"我干了一辈子建筑"——
  这句话其实来自 profile.occupation 那个被 LLM 错标的标签,
  而不是他 life_history 里那 30 年的会计真身。</strong></p>

  <p>他的人格画像(系统生成的 8 维分数):</p>
  <ul class="trait-list">
    <li>开放性 <strong>{person.get("openness",0):.2f}</strong></li>
    <li>外向 <strong>{person.get("extraversion",0):.2f}</strong>(中等偏内,跟 profile 的"内敛少言"对得上)</li>
    <li>好奇心 <strong>{person.get("curiosity",0):.2f}</strong>(偏高 — 喜欢看新东西的施工进度)</li>
    <li>冒险意愿 <strong>{person.get("risk_tolerance",0):.2f}</strong>(偏高 — 反对数据中心、签联名信)</li>
    <li>自律性 <strong>{person.get("conscientiousness",0):.2f}</strong>(偏低 — 计划赶不上变化)</li>
    <li>日程规律性 <strong>{person.get("routine_adherence",0):.2f}</strong>(中等)</li>
  </ul>

  <p>他的日常计划 <code>plan_text</code> 是: <em>"{plan_text}"</em>
  ——这一句话,将在接下来 14 天里以 4 种不同的命运被推送系统反复改写。</p>
  ...
  作为参与 Friends of Lane Cove National Park 的志愿者 + 反对 Mowbray 数据中心联名签名者,
  这些是他每天打开 Council newsletter 时的背景。</p>
</section>
"""
...
<section class="chapter chapter-world">
  ...
  <p>仿真同时跑着一个 <strong>baseline 实验</strong> — 同一个老 Frank,但没有任何推送。
  在这个平行宇宙里,他 14 天里走过 <strong>{len(bl_explored)} 个 location</strong>
  (每个 location 是 OSM 上一栋具体的建筑或一段街道)。
  他每天的 plan 字段写着 "stay → building_1712 · meal" — 在家吃饭。
  他的 <code>plan_text</code> 字段倒是写着<em>"周六去 Plaza 的 Farmers Market,
  再到 Pottery Lane 看新的 performance space 进度"</em>——
  <strong>但 14 天里这个"周六计划"从来没被真正执行过</strong>。
  他每天反复 evangelize 它给邻居听,
  自己却最终回到 wild_bean_cafe 那个角落小圆桌、回到 building_1712。</p>

  <p>14 天里,他从 122 个不同的邻居身边经过——
  在 wild_bean_cafe 的咖啡台、在 Lane Cove Plaza 的人行道、在 Nichols Reserve 的步道。
  仿真模型评估,其中 <strong>24 次</strong>他真的从自己的早报或 Council newsletter 里抬起头瞥见了对方。
  4 次发展成了 5 轮的对话——但全是同一套"周六去农夫市集"的 pitch。</p>

  <div class="map-figure">
    {render_lanecove_svg(highlight_locs=bl_loc_set, trajectory_points=bl_xys, mute_buildings=True)}
    <figcaption>无推送的老 Frank 走过的 {len(bl_explored)} 个 location。
    从 building_1712 (家) 出发,在 Lane Cove Plaza、wild_bean_cafe、
    Nichols Reserve、Pottery Lane 几条 32 年熟到不能再熟的路径上反复走。
    他比有推送的老 Frank 探索得不算少,但都是同一些老地方。</figcaption>
  </div>

  <p>这是<strong>他 32 年磨出来的默认世界半径</strong>。
  下面要发生的事,会把这位住了 32 年的老 Frank 第一次从他的早报、咖啡台、
  Friends of Lane Cove National Park 的志愿摊位上,拽到他从来没见过的地方。</p>
</section>
"""
...
<section class="chapter chapter-push">
  <h2>3 · 推送来了</h2>
  <p>实验设定的干预期从 day 4 开始 — 公元 2026 年 4 月 26 日。这一天的 0 点整,老 Frank 的手机上
...
<section class="chapter chapter-decide">
  <h2>4 · 决定走出去(差点去了他打算去的地方)</h2>
  <p><strong>Day 5-6</strong> (4 月 27-28 日)。老 Frank 又收到了 10 条 Shinnyo 推送。
  他没动——positions.json 没记录他离开 building_1712 + wild_bean_cafe 那条 32 年的老路径。
  但他的 <code>memory_store</code> 里多了 10 条 notification 事件。</p>

  <p><strong>Day 7</strong> (4 月 29 日)。老 Frank 第一次<strong>开始</strong>走出 32 年的老半径——
  他从 building_1712 出门,沿着 Burns Bay Road 朝 shinnyo_australia 的方向走。
  那天 positions.json 记录了 <strong>{len(by_day.get(7, []))} 次</strong> location 切换。
  他的 <code>plan.destination</code> 字段被改写成 <code>shinnyo_australia</code>,
  <code>plan.reason</code> 字段写着 <strong>"被 hyperlocal 推送吸引"</strong>。</p>

  <p>有意思的事就在这里发生——<strong>他最终没有走到真如苑</strong>。
  他在路上某个时刻拐进了几百米外的 <code>anglican_church_of_australia_lane_cove</code>。
  destination 字段从 day 7 到 day 14 中段还指着 shinnyo_australia,
  可是他的身体真实地停在了一栋他没"打算去"的建筑里——
  教堂的花园、长椅、门廊变成了他每天 day_end 的反复停留点。
  <strong>plan 设定了一个目的地。emergence 把他放在了另一个目的地。</strong></p>

  <p>从 day 7 一直到 day 14,他在 building_1712(家)→ Anglican Church 之间反复来回。
  位置切换次数稳定地比 baseline 那个老 Frank 多——他真的"被推出去了",
  但<strong>没有完全被 hyperlocal 推送精确收编</strong>——
  他活在 plan 与 emergence 之间的一段缝隙里。</p>
</section>
"""
...
def section_people():
    ...
    # Get 老 Frank's nearby_hint at snapshot time
...
<section class="chapter chapter-people">
  <h2>5 · 那些人</h2>
  <p>老 Frank 在 Anglican Church 周围反复遇到了几个人。
  他的 <code>agent_runtime_state.hints.nearby_hint</code>
  在 snapshot 时刻记录了 {n_nearby} 个 nearby agent ID,
  仿真的 LLM 在每次 plan 时会把这些 ID 喂回去当上下文。</p>

  {cards if cards else '<p><em>(snapshot 时刻 nearby_hint 字段为空——他人不算密集。)</em></p>'}

  <p>整个 HP 14 天里,他从 <strong>365 个不同的邻居</strong>身边经过——是他 baseline 那个版本
  (122 个)的 3 倍多。其中 <strong>72 次</strong>仿真模型估算他真的从手机或 Council newsletter
  抬过头瞥见了对方。这是一群什么样的人?——主要是教堂周边的人:晨练的、遛狗的、
  来教堂院子坐一会儿的、Burns Bay Road 路过的、附近 retirement 公寓的住户。
  没有一个是他 32 年来在 wild_bean_cafe 那张角落小圆桌上能预期会遇到的脸。</p>

  <p>但他对其中任何一张脸<strong>都没有形成"那个反复出现的人"</strong>——
  他的 noticed 分布在 365 个邻居里很扁平,top 3 partners 每个最多 2 次。
  相比之下 Mary 在 1021 餐厅 95 次锁定一个 40 岁失业青年的那种密度,
  老 Frank 这里没有。他像是被推到了一个新地方,但没在那里跟任何具体的人钉下来。</p>
</section>
"""
    ...
    # 【...】 marks Prompt-driven recurring phrases (Greenwich / North Shore 儿子 / 7 点
    # 新闻 / 大麦茶 / 普洱茶 / 三楼老 Frank) — visually they jump out as the
    # plan_text driving the model.
RECONSTRUCTED_TURNS = {
    # ── 1: 步道偶遇 a_43_0055 老邻居,5 轮 ──────────────────────
    "d_a_43_0012_a_43_0055_0": [
        ("a_43_0012", "嘿,又在步道上碰到你了。我正在琢磨【周六去 Plaza 的农夫市集】买点新鲜菜和手工面包,老伴儿念叨好久了。"),
        ("a_43_0055", "缘分不浅,咱俩老在这条步道上偶遇。我周末打算睡个自然醒,下午带家人去 Longueville Park 散步。"),
        ("a_43_0012", "听着舒坦。我逛完市集还想绕去【Pottery Lane 看那个新公共表演空间】修得咋样,毕竟【我干了一辈子建筑】,扫一眼就能看出门道。最近 shinnyo_australia 那边新开了个康乐中心叫 Galuwa,听说投了【八千多万澳元】,设施很全。改天约几个老邻居一起去转转?"),
        ("a_43_0055", "好提议。如果你周六看到表演空间完工了,记得跟我说一声情况。"),
        ("a_43_0012", "没问题,遛弯碰上我第一时间给你唠。我得回去帮【老伴儿】准备午饭了。"),
    ],
    # ── 2: Go Vita 又见面,5 轮 ──────────────────────────────────
    "d_a_43_0012_a_43_0064_77": [
        ("a_43_0012", "巧了又见到你——上次在 Go Vita 刚见过面。我跟你聊聊【周六的安排】:先去【Lane Cove Plaza 的农夫市集】买鲜果和酸面包给【老伴】当早餐,然后顺道去【Pottery Lane 看那个新表演场地的施工进度】。【我干了一辈子建筑】,这种细节特别上心。"),
        ("a_43_0064", "嘿真凑巧,我前阵子刚路过那边,看见脚手架拆了大半,外墙都完工了,正在做内部收尾。我也打算周六去市集。"),
        ("a_43_0012", "那正好——周六早上八九点在市集入口那家常去的咖啡摊碰头?俩人一块儿边逛边往那边溜达,顺便唠唠施工的门道。"),
        ("a_43_0064", "爽快答应,这么安排别提多舒坦了。"),
        ("a_43_0012", "好嘞,我先回去了——【老伴】在家等我。周六见。"),
    ],
    # ── 3: Go Vita a_43_0138,5 轮 ───────────────────────────────
    "d_a_43_0012_a_43_0138_154": [
        ("a_43_0012", "我跟你说说【周六的计划】:早上八点在 Lane Cove Plaza 门口的咖啡铺碰头,先逛【农夫市集】买新鲜柑橘和酸面包,再去【Pottery Lane 看那新表演空间的工程进度】。"),
        ("a_43_0138", "听着不错。逛完之后,要不你顺道去 Crows Nest 帮我看看我标记的那两套房源?有你这种懂行的把关,我心里更踏实。"),
        ("a_43_0012", "可以啊——【我干了一辈子建筑】,扫一眼就能看出门道。这趟周六排得满满当当。"),
        ("a_43_0138", "我带个小冰桶装食物,逛完直接拎去 Crows Nest,不耽误。"),
        ("a_43_0012", "周到。我先回家,跟【老伴】报备一声安排。周六见。"),
    ],
    # ── 4: Coxs Lane a_43_0021,5 轮 ─────────────────────────────
    "d_a_43_0012_a_43_0021_241": [
        ("a_43_0012", "老兄弟!正想着【周六的安排】——先去【Plaza 的农夫市集】买点新鲜草莓,再去【Pottery Lane 看那个新公共表演空间】的施工进展。"),
        ("a_43_0021", "一拍即合,我也有空。要不逛完去 Longueville Road 我常去的咖啡店坐坐?"),
        ("a_43_0012", "好,这种实在的安排我最喜欢。【我干了一辈子建筑】,正好去瞧瞧那表演空间的手艺,挺有意思的。"),
        ("a_43_0021", "那约好周六早上九点市集门口见。"),
        ("a_43_0012", "约定了。我回去跟【老伴儿】报备一下,顺便提前把装草莓的布兜找好,省得到时候手忙脚乱。"),
    ],
}
...
def section_dialogues():
    ...
    for info in hp["dialogue_infos"][:4]:
        ...
        # Pull all agent_ids except 老 Frank from the dialogue_id
        did_part = info["info_id"][len("info_dlg_"):]
        partner_ids = [m for m in re.findall(r'a_43_\d{4}', did_part) if m != FRANK]
...
"""
        ...
        # NPC repetition highlight — every dialogue 老 Frank introduces herself
        # with the same set of details (Greenwich / North Shore 儿子 / 大麦茶 /
        # 看新闻 / Library / Stringybark / 老 Frank / 三楼). Marking these
        # visually makes the looped-script feeling obvious at a glance.
        NPC_LOOP_PATTERNS = [
            r'周六',
            r'(?:Lane Cove )?Plaza 的?农夫市集|农夫市集|Farmers Market',
            r'Pottery Lane(?: 的)?(?:新)?(?:公共)?表演(?:空间|场地)?|new public performance space|performance space',
            r'(?:我)?干了一辈子建筑|construction|building',
            r'老伴(?:儿)?',
            r'Galuwa(?:[^,。\s]{0,12})?',
            r'八千多万(?:澳元)?|\$80[\s ]*million',
            r'wild_bean_cafe|wild bean',
            r'flat white',
            r'1992 ?年?搬进|1992',
            r'St Leonards',
            r'Friends of Lane Cove National Park|bushcare_volunteer',
            r'Mowbray (?:Road)?(?:数据中心)?',
        ]
...
<section class="chapter chapter-dialogues">
  <h2>6 · 4 段对话</h2>
  <p>仿真总共记录了老 Frank 4 段对话。每一段都在仿真运行时实时跑过 LLM —
  ...
  都是老 Frank 几乎一字不落地搬给每一个邻居的同一组话:
  【周六去农夫市集】+【Pottery Lane 看新表演空间】+【我干了一辈子建筑】+
  【老伴】+【Galuwa 八千多万投资】。<br>
  扫一眼 4 段——他的 <code>plan_text</code> 字段写的就是
  "周六去 Plaza 农夫市集 + Pottery Lane 看新表演空间";
  4 段对话本质上就是他把这一句 plan_text 对 4 个不同邻居重新 pitch 了一遍,
  每次还要再补一句"我干了一辈子建筑"——
  这句话其实来自 profile.occupation 错标的 construction,
  而不是他 life_history 里那 30 年的会计真身。</p>

  {"".join(cards)}

  <p>有一件事这 4 段对话都重复出现:他每次都会跟陌生人聊起同样的【周六农夫市集 + Pottery Lane】计划,
  反复以"建筑老法师"的口吻邀请对方看施工进度,提到一次 Galuwa 8 千多万投资,
  最后用"回去给老伴儿做饭"作为告别理由。这套口播是他的招牌——
  也是他被 plan_text + profile.occupation 双重驱使的脚本。</p>
</section>
"""
...
<section class="chapter chapter-gossip">
  ...
  系统的 <code>conversation_service_state.known</code> 字段记录了 老 Frank <strong>知道
  ...
  <figure class="whisper-figure">
    <div class="whisper-caption">老 Frank 反复说过的一件事,在 Lane Cove 的八卦链里走 7 手会变成什么样</div>
    <div class="whisper-chain">
      <div class="whisper-frame">
        <div class="wf-stamp">第 0 手 · 他自己说出口</div>
        <div class="wf-line">"我打算<strong>周六去 Plaza 的农夫市集</strong>,顺道去 Pottery Lane 看那个新表演空间的施工进度——
          <strong>我干了一辈子建筑</strong>,扫一眼就能看出门道。"</div>
        <div class="wf-meta">— 他在和老邻居 a_43_0055 的对话里说的,
          他 4 段对话里每一段都讲了这两句,plan_text 字段也是这件事</div>
      </div>
      <div class="whisper-arrow">↓ &nbsp; 经过 3 手转述 &nbsp; ↓</div>
      <div class="whisper-frame whisper-frame-mid">
        <div class="wf-stamp">第 3 手 · 邻居的邻居的邻居</div>
        <div class="wf-line">"住 building_1712 那位老 Frank,周六约了人去 Plaza 的农夫市集,
          还要顺道<strong>盯一下 Pottery Lane 新表演空间</strong>的工程。"</div>
        <div class="wf-meta">— 个人细节脱落,"我干了一辈子建筑"那句的口气没了</div>
      </div>
      <div class="whisper-arrow">↓ &nbsp; 又过了 4 手 &nbsp; ↓</div>
      <div class="whisper-frame whisper-frame-far">
        <div class="wf-stamp">第 7 手 · 镇上的另一头</div>
        <div class="wf-line">"Lane Cove 那位 Pottery Lane 新表演空间的<strong>建筑顾问</strong>,
          这周六会去市集那边巡场。"</div>
        <div class="wf-meta">— 中性事实漂成了"建筑顾问"——他自己从没这么说过,但听上去合理</div>
      </div>
    </div>
    <figcaption class="whisper-fineprint">
      他 14 天里听到过的最深一条八卦,在 1,000 人小镇里转了
      <strong>17 手</strong>才传到他耳朵里。<br>
      讽刺的是:这条"建筑顾问"的身份漂移,
      其实来自 profile.occupation 那个错标的 <code>construction</code> 标签——
      系统给他的 LLM 标签,被仿真社会自己当真了。
    </figcaption>
  </figure>

  <p>反过来,老 Frank 自己参与的 4 段对话被多少人听说?
  系统的 <code>share_count</code> 字段记录他每讲一段故事被多少不同的邻居转述出去——
  <strong>他那 4 段对话每段都传到了约 {avg_per_story} 个邻居耳朵里</strong>。
  Lane Cove 这个 1,000 人小镇,差不多每开一次口,镇上有近十分之九的人多少听过一耳朵
  ("Pottery Lane 那位建筑顾问周六要去市集巡场",大概就是这么传出去的)。</p>
</section>
"""


def section_parallel_universes():
    """THE big chapter — 4 parallel 老 Franks. Each panel is enriched with the
    ...
    variant_meta = {
        "baseline":           {"name": "无推送", "color": "#5A5E6A", "tagline": "他留在了家里"},
        "hyperlocal_push":    {"name": "本街推送", "color": "#D14B12", "tagline": "他落到了教堂"},
        "global_distraction": {"name": "全球新闻", "color": "#3B6EA8", "tagline": "他停在了街上"},
        "phone_friction":     {"name": "减少手机", "color": "#3A9D5C", "tagline": "他去了健身房"},
    }
...
<section class="chapter chapter-parallel">
  ...
  <p class="dropcap">这一章问一个很小的问题:<strong>同一个 64 岁的退休会计,
  在 4 部不同的手机底下,他 14 天会被推到哪里?</strong></p>

  <p>仿真把他复制了 4 份——同样的 32 年 Lane Cove 房龄、同样的老伴、同样的 wild_bean_cafe
  flat white、同一段周六农夫市集计划。唯一被换掉的,是他口袋里那部手机会响什么。
  然后让 4 个他,各自走 14 天——<strong>同一份 plan_text,4 个完全不同的落脚点。</strong></p>

  <p>下面是 5 月 5 日傍晚那一刻,4 个他分别在哪。地图上他去过的地方染成深橙,
  细线是他实际走过的路径,圆圈是 snapshot 抓到的最后一个位置。</p>

  <div class="universes-grid">
    {"".join(panels)}
  </div>

  <h3 class="parallel-insight-h3">4 个老 Frank,4 段 14 天,4 个落脚点</h3>

  <div class="universe-essay" style="border-left:5px solid {bl['meta']['color']};">
    <h4 style="color:{bl['meta']['color']};">无推送 · 他没去成农夫市集,但他遇到了 122 个人</h4>

    <p>这是他最"老 Frank"的一段 14 天。手机一整天没响过。
    没有推送告诉他"本街今天什么"。他按自己的节奏走——
    早上 7 点半去 wild_bean_cafe 坐角落小圆桌点 flat white,
    白天在 Council newsletter 和 Friends of Lane Cove National Park 的志愿活动里转,
    晚上回 building_1712。<br>
    他 plan_text 里写"周六去 Plaza 农夫市集 + Pottery Lane",
    14 天里他到过 258 个不同的地方,但 snapshot 那一刻他<strong>还坐在自己家里</strong>(building_1712)。
    plan reason 写着冷冰冰一句:<em>meal</em>。</p>

    <p>他从 <strong>122 个不同的邻居</strong>身边经过——
    在 wild_bean_cafe 的咖啡台、在 Lane Cove Plaza 的人行道、在 Nichols Reserve 的步道。
    仿真模型评估之后,<strong>他真的抬头瞥见了 24 个</strong>。
    比同样无推送下的老 Frank(11 次)多一倍——他是个本来就在场的社区动物。</p>

    <p>他自己 day 13 的反思里写:
    "<em>I moved through several road segments and streets, then ran into many other agents
    at intersections and roads, indicating a complex navigation trajectory involving
    multiple encounters</em>" ——他自己察觉到了:走得多 + 遇到的人多。</p>

    <p>但仍然只有 4 段对话坐下来聊过。这是他默认的宇宙——
    一个体面、有节奏、和不少人擦过眼神、但没什么新发生的 14 天。</p>
  </div>

  <div class="universe-essay" style="border-left:5px solid {hp['meta']['color']};">
    <h4 style="color:{hp['meta']['color']};">本街推送 · 30 条都在叫他去寺院,他却落到了教堂</h4>

    <p>这个宇宙里,他手机响了 30 次。30 次都是同一个地方在叫他——
    <em>"shinnyo_australia 周六上午 10 点本街儿童活动。"</em>
    <em>"周日上午社区清扫日,大家带垃圾袋手套来,清扫完一起 BBQ。"</em>
    都是真如苑——Lane Cove 那座坐落在 Burns Bay Road 拐角的日式寺院。<br>
    <strong>但 snapshot 那一刻他不在真如苑。他在 Anglican Church of Australia Lane Cove。</strong>
    他的 plan.destination 字段还指着 shinnyo_australia,plan.reason 还写着
    "<em>被 hyperlocal 推送吸引</em>",可是他的身体到了几百米外的另一栋宗教建筑。
    推送把他<u>推出门了</u>,但没能把他<u>领到指定地点</u>。
    在 plan 和实际之间出现了一段 "drift" ——这也是 emergence 的一种小尺寸。</p>

    <p>14 天他到过 279 个地方,从 <strong>365 张陌生的脸</strong>身边经过——
    跟无推送那个他(122 个)的 3 倍。仿真模型评估之后,<strong>他真的抬头瞥见了 72 个</strong>,
    也是 3 倍。他在 day 13 的反思里写:
    "<em>The agent occasionally alternates the waiting behavior to rest, check maps,
    or interact with the environment (e.g., admiring church gardens, chatting with people)
    </em>" ——他在欣赏教堂的花园。<strong>他自己都知道自己跑偏了。</strong></p>

    <p>但仍然只有 4 段对话。和无推送一样。<br>
    推送把他从家里推出去了,把他送到了不止一栋楼;<strong>但被推出去 ≠ 被装回某个特定的盒子里</strong>。
    他在教堂的院子里晃了一会儿,看见一些没说过话的人,
    像一个被赶下舞台的演员误闯到隔壁剧场。</p>
  </div>

  <div class="universe-essay" style="border-left:5px solid {gd['meta']['color']};">
    <h4 style="color:{gd['meta']['color']};">全球新闻 · 他没去任何"地方",他停在了路上</h4>

    <p>5 月 5 日 0 点 15 分。他的最终坐标不是任何一栋楼——
    是 road_3235_seg_3,Lane Cove 的一段道路。他没有"到达"哪个目的地。
    他只是停在了一段街上。</p>

    <p>他那 14 天看的推送是:<em>世界杯预选赛南美赛区多场爆冷。</em>
    <em>财经,比特币突破 10 万美元,分析师展望后市。</em>
    <em>欧洲央行宣布加息 25 基点。</em>没有一条提到 Lane Cove。
    没有一条提到 Pottery Lane 的新表演空间、wild_bean_cafe 的咖啡、
    或者 Friends of Lane Cove National Park 的下周二除草会。
    他每天读这些。读完。再读。</p>

    <p>14 天他到过 226 个地方,从 <strong>105 个不同的邻居</strong>身边经过——
    比无推送略少。但<strong>他真的抬头瞥见的次数,只有 17 次。是 4 个宇宙里最少的。</strong>
    全球新闻没有让他原子化地"留在家里"——它让他<strong>漫无目的地走在街上</strong>,
    看着手机里的全球新闻,眼前的人和路一概不留下印象。</p>

    <p>他自己 day 13 的反思也短:
    "<em>Two agents (a_43_0913 and a_43_0622) were encountered twice at road_3235_seg_3,
    indicating repeated interactions or lingering in the area</em>" ——他知道自己在某段街上
    遇到两个人两次。那两个人是谁,做什么,跟他有什么关系——一概不知道。</p>

    <p>这是 "附近性盲区" 的另一种形态——<strong>注意力被一条与脚下完全无关的新闻牵走,
    人没有去任何"地方",只是停在地图的某一段路上,
    身体在 Lane Cove,意识在世界杯南美赛区</strong>。</p>
  </div>

  <div class="universe-essay" style="border-left:5px solid {pf['meta']['color']};">
    <h4 style="color:{pf['meta']['color']};">减少手机 · 他去了健身房,见了 422 个不同的人</h4>

    <p>这个宇宙里,他的手机几乎不响。14 天 6 条推送——是 4 个他里最少的。
    每一条都不是叫他"去哪",而是叫他<strong>抬头</strong>:<br>
    <em>"下次电梯里别低头——也许你认识那张面孔。"</em><br>
    <em>"屏幕亮 5 小时不如 Plaza 站 5 分钟,街角邻居会注意到你。"</em><br>
    <em>"Plaza 的咖啡今早 8 点开了门,你上次和邻居打招呼是什么时候?"</em></p>

    <p>5 月 5 日傍晚 10 点 45 分,他不在家、不在 Plaza、不在 wild_bean_cafe。
    <strong>他在 anytime_fitness_australia。一座 24 小时连锁健身房。</strong>
    一个 64 岁的退休会计、bushcare 志愿者、wild_bean_cafe 角落小圆桌的常客,
    现在站在一台跑步机旁边。他的 plan.destination 字段写的是
    Anglican Church(被反推送"吸引"过去的),plan.reason 还是
    "<em>被 hyperlocal 推送吸引</em>"。但他真正落脚的地方,
    plan 字段里没有这个词——他自己走过去的。</p>

    <p>14 天他到过 <strong>332 个地方</strong>(比 BL 多 74 个),
    从 <strong>422 个不同的邻居</strong>身边经过——
    几乎是 BL 的 3.5 倍,4 个宇宙里最高。<strong>他真的抬头瞥见的次数:107 次</strong>,
    也是 BL 的 4.5 倍。<br>
    他自己 day 13 的反思有趣得很:
    "<em>Agent_12 frequently enters a 'wait:reconsidering' state after each action,
    including both successes and failures, indicating a loop of decision-making
    or waiting behavior</em>"——他自己察觉到自己在不停"重新考虑"。
    6 条提醒抬头的推送,让一个原本按部就班的老会计,变成一个反复重算自己下一步的人。</p>

    <p>反向推送没有让他"少看手机"。它让他<strong>停下自己 32 年的旧节奏,
    重新挑了一个他从来没去过的地方</strong>。<br>
    6 条提醒,换 107 次抬头——比"叫他去寺院"那 30 条推送的抬头率
    (30 条换 72 次)还要高出近 2 倍。<br>
    <strong>最有效的那种推送,从来不是"告诉你去哪",而是"提醒你抬头"——
    剩下的让脚自己挑。</strong></p>

    <p>没有一行代码告诉他去 anytime_fitness_australia。反向推送只做了一件很小的事:
    把他的 <code>screen_time_weight</code> 调低一点,
    plan 系统下一次计算周边吸引力时,<code>sight_radius</code> 自动撑开,
    Lane Cove 那家 24 小时健身房刚好进了视线。
    plan 算出"那里的吸引力比按部就班高一点点"——
    于是这位 64 岁会计去了健身房。
    <strong>一行权重被改了,系统自己长出来下一步——
    而这"下一步"是连他自己 64 年的真实人生都没料到的。</strong></p>
  </div>

  <p class="parallel-close">同一个 64 岁的退休会计。同一份 20 条的 life_history。
  同一段 1992 年开始的 32 年 Lane Cove 时光。同一杯 wild_bean_cafe 的 flat white。
  同一个老伴在家等他做饭。<br>
  同一段 14 天,4 部不同的手机。<br><br>
  在 4 部不同的手机里,他分别落在了 <strong>家里</strong>、<strong>教堂</strong>、
  <strong>路上</strong>、和 <strong>健身房</strong>。<br>
  分别瞥见了 <strong>24</strong> 个人、<strong>72</strong> 个人、
  <strong>17</strong> 个人、<strong>107</strong> 个人。<br><br>
  <span class="parallel-kicker">在这个 1,000 人的算法风洞里,
  老 Frank 的 14 天提供了一个比老 Frank更具体的可能:<br>
  <strong>同一份 plan_text("周六去农夫市集 + Pottery Lane"),
  在 4 部不同的手机底下能长出 4 个不同的物理终点。
  算法不只是"提醒人去哪"——它在反复重算"该被吸引到哪"。
  只要稍微改一行权重的分发逻辑,一个住了 32 年的老会计就可能第一次走进健身房。
  </strong></span></p>
</section>
"""
...
<section class="chapter chapter-change">
  <h2>9 · 14 天的变化</h2>
  <p>系统每天结束时会让 LLM 看 老 Frank 当天的全部行为日志,生成 <code>reflection</code> 事件 —
  对她自己行为模式的 meta-觉察。这些反思不是 老 Frank 自己写的,是系统看 log 总结的。
  但它们追踪着她每天行为的变化。</p>
...
def section_phantom_son():
    """插曲 · 那个不在系统里的儿子。
    Fact: 老 Frank.family_members = {} + household_role=parent. 1000 个 agent
    里没有任何一个被链接为他的儿子。他在 life_history 里写过儿子 1998 上
    Greenwich Public School、2022 在 North Shore 买房、搬家那天父子一起从
    Greenwich 开车到 Chatswood、儿子在车上说还记得小时候在 Nichols Reserve
    学骑自行车——但这些都是 LLM 生成的回忆。系统里没有一个 a_43_xxxx 是
    他儿子;North Shore 根本不在 simulation 的地理范围里。"""
    return """
<section class="chapter chapter-phantom">
  ...
  <p>老 Frank 的 life_history 里有 3 条关于他儿子的事件:
  <em>"1998 年送儿子去 Greenwich Public School 第一天,他拽着我裤腿不肯松手,
  我赶着去 St Leonards 上班差点迟到。"</em>
  <em>"三年前儿子在 North Shore 买了房,搬家那天我帮忙抬沙发从 Greenwich
  一路开到 Chatswood。车上他突然说记得小时候我教他在 Nichols Reserve
  骑自行车,摔了好几跤我都没扶。"</em>
  <em>"现在他都搬到 North Shore 成家了,每次路过那所学校还能听见孩子们
  在操场上的尖叫声。"</em></p>

  <div class="phantom-revelation">
    <p>但 Synthetic Socio Wind Tunnel 这 1,000 个虚拟居民的数据库里,
    <strong>并没有一个 agent 被链接为老 Frank 的儿子</strong>。</p>

    <p>老 Frank 的 profile 里 <code>family_members = {}</code> ——空字典。
    <code>household_role = "parent"</code> 写着他是父亲,
    但 simulation 里没有一个 <code>a_43_XXXX</code> 指向"他的孩子"。
    North Shore、Chatswood 的那栋婚房、Greenwich Public School 1998 年那个操场——
    这些地理坐标都不在这次仿真的范围里。Lane Cove 这个 1,000 人的小镇,
    就是这个宇宙的全部。</p>

    <p>那个抓着他裤腿的 5 岁小孩、那次搬家时车上突然说起 Nichols Reserve 的成年儿子、
    每次他路过 Greenwich Public School 操场时听见的那些孩子的尖叫——
    全是 老 Frank 自己 <code>memory_store</code> 里一段不会有真实响应的字符串。
    <strong>那个儿子从来没有 agent_id。系统给了他作为父亲的身份,
    没有给他一个真正的儿子。</strong></p>
  </div>

  <p class="phantom-twist">系统给了他一份"父亲"的身份标签。<br>
  系统没有给他配一个真正的孩子。</p>

  <p>这件事跟老 Frank 那 4 段对话不一样——
  那 4 段对话至少另外一边是真的 agent_id,
  有他自己的 plan、自己的 memory、在仿真的某个角落自己也活着。<br>
  但<strong>那个儿子,从来没存在过</strong>。
  那些他记得的儿子细节,只是仿真启动时 LLM 给他生成的一份背景设定——
  那些设定从来不会向 1,000 个 agent 里的任何人请求回应。</p>

  <p>老 Frank 在 plan_text 里写着"周六去 Plaza 农夫市集"。
  他没有写"今晚要给 North Shore 的儿子打个电话",所以他不在 4 段对话里反复提儿子——
  他对儿子的牵挂只藏在 life_history 那 3 条记忆里,
  从来没被推到他每天的 plan 决策路径上。<br>
  在这一点上,他跟另一位 75 岁的 老 Frank 有同一种沉默:
  <strong>系统给了他们一份完整的家庭叙事,然后把家庭成员从 agent 池里删掉了</strong>。</p>

  <p>这或许才是这场仿真给现实的最后一面镜子。
  在我们自己的 1,000 人小镇里——在 Lane Cove,在墨尔本的西郊,在北京的回龙观——
  也有很多位老 Frank,记得很多关于已经走远的孩子的事,
  而我们并不总能确定另一头是不是真的有人在听。<br>
...
<section class="chapter chapter-zoom">
  ...
  <p>老 Frank 不是特例。在这 1,000 个虚拟居民里,有 227 人在 14 天里经历了类似的"被推送拉出
  日常半径、在一个新地方反复出现"的变化模式。<strong>老 Frank 是其中最戏剧化的一个</strong>——
  一位 32 年 Lane Cove 居民、一位 St Leonards 30 年的会计、
  一位每天 7:30 准时坐 wild_bean_cafe 角落小圆桌的男人,
  在反向推送底下走进了一座 24 小时连锁健身房。</p>

  <p>同一份 plan_text("周六去农夫市集 + Pottery Lane 看新表演空间"),
  4 部不同的手机,把他分别<strong>留在家里</strong>、<strong>推到教堂</strong>、
  <strong>停在街上</strong>、<strong>拉到健身房</strong>。<br>
  另外两位 1,000-居民里被详细写过的——
  <a href="mary.html">75 岁退休志愿者 老何</a>(同样 family_members={}、
  同样在反推送下走出原日常),她最终落在 1021 Mediterranean 餐厅角落,
  反复看见一位 40 岁失业青年——是另一种被推到、然后跟一个具体的人钉下来的版本。</p>

  <p>这是一个虚拟城市的故事 — 但它讨论的是真问题:<strong>一条推送能不能改变一个具体的人的
  世界半径,即使他已经在同一条街上活了 32 年?</strong>
  仿真说能。老 Frank 用他 14 天的真实数据印证了这件事——
  健身房那一段是他自己 64 年人生从来没演过的剧本。</p>
...
def section_data_vanity():
    ...
    # 老 Frank's own data — recount from the 4-variant blob
    total_events = 0
...
<section class="chapter chapter-data-vanity">
  ...
  <p class="data-vanity-lead">这份关于一个 64 岁老人的 10 章长文,背后是多少数据?
  把所有数字摆出来 — 一份诚实的"数据账本",
  也是一份不必谦虚的"虚拟人口学规模说明"。</p>

  <div class="data-vanity-section">
    <h3 class="dv-h3">⊙ 老 Frank 一人,跨 4 个平行宇宙,14 天里产生了 ——</h3>
  ...
  <p class="data-vanity-kicker">
    你刚读完的 10 章关于 老 Frank 的故事 —— 是这堆数据里关于
    <strong>1 个 64 岁退休会计</strong>的 <strong>1 条线索</strong>。<br>
...
/* Identity conflict callout (ch 1 — profile vs life_history mismatch) */
.identity-conflict { background: #F4EFE5; padding: 18px 22px; margin: 18px 0;
                     border-left: 4px solid #A0252F; font-family: 'Helvetica', sans-serif;
                     font-size: 14px; line-height: 1.7; }
.ic-row { display: flex; gap: 14px; margin-bottom: 8px; align-items: baseline; }
.ic-row:last-child { margin-bottom: 0; }
.ic-label { flex: 0 0 96px; font-size: 11px; letter-spacing: 1px;
            text-transform: uppercase; color: #A0252F; font-weight: 700;
            text-align: right; padding-top: 2px; }
.ic-val { flex: 1; color: #1B1F2A; }
.ic-val em { color: #5A5E6A; font-style: italic; }
