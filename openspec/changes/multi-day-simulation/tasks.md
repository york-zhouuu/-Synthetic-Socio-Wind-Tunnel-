...
- [ ] 1.1 `synthetic_socio_wind_tunnel/orchestrator/models.py`：在
  `TickContext` / `TickResult` / `CommitRecord` / `SimulationSummary`
  新增 `simulated_date: date` 与 `day_index: int = 0` 字段；frozen 约束不变
- [ ] 1.2 `synthetic_socio_wind_tunnel/memory/models.py`：`MemoryEvent`
  新增 `day_index: int = 0` 字段
- [ ] 1.3 新建 `synthetic_socio_wind_tunnel/memory/carryover.py`：定义
  `CarryoverContext` Pydantic frozen 模型（字段见 spec）

## 2. MultiDayRunner 核心

- [ ] 2.1 新建 `synthetic_socio_wind_tunnel/orchestrator/multi_day.py`：
...
- [ ] 2.2 让 `MultiDayRunner` 自动 wire：若 memory_service 传入，每 day 末
  调 `run_daily_summary`；若 planner 传入，每 day 初生成 plan 并把
  carryover 注入

## 3. Orchestrator 多日参数

- [ ] 3.1 `orchestrator/service.py::Orchestrator.run` 签名加
  `*, day_index: int = 0, simulated_date: date | None = None` kwarg
- [ ] 3.1a 内部把 `day_index` / `simulated_date` 填入所有
  TickContext / TickResult / CommitRecord
- [ ] 3.2 **向后兼容验证**：所有现有 `Orchestrator.run()` 无参调用 SHALL
  继续工作，day_index=0 / simulated_date 从 Ledger.current_time 派生

## 4. Memory 跨日接口

- [ ] 4.1 `memory/service.py`：
...
- [ ] 4.2 `memory/service.py::process_tick`：把 `tick_result.day_index`
  传递给派生的所有 MemoryEvent
- [ ] 4.3 `run_daily_summary` 产出的 `daily_summary` 事件写入
  `MemoryStore` 时 day_index 必须正确填充

## 5. Planner 跨日 prompt

- [ ] 5.1 `agent/planner.py::generate_daily_plan` 签名加 `carryover:
  CarryoverContext | None = None` kwarg
- [ ] 5.2 `_PLAN_PROMPT_TEMPLATE` 扩展为两段：原 prompt + 可选 carryover
  段（若 carryover=None 则省略整段；行为与单日完全一致）
- [ ] 5.3 Prompt 构造时实现 1500 字符 cap + summary_text 300 字符截断
  （见 spec Scenario）

## 6. CLI 入口

- [ ] 6.1 新建 `tools/run_multi_day_experiment.py`：
  - argparse: `--start-date / --num-days / --agents / --seeds /
    --variant / --mode`
  - 循环 seed → 跑 MultiDayRunner → dump JSON
  - aggregate 调 `MultiDayResult.combine`
- [ ] 6.2 在 `tools/smoke_experiment_demo.py` 中加 `--multi-day` 开关，
  触发多日路径（默认仍跑单日，保持向后兼容）

## 7. 公共 API re-export

- [ ] 7.1 `synthetic_socio_wind_tunnel/__init__.py` 导出：
  - `MultiDayRunner` / `MultiDayResult` / `MultiDayAggregate`
  - `CarryoverContext`
- [ ] 7.2 `synthetic_socio_wind_tunnel/orchestrator/__init__.py` 同步
- [ ] 7.3 `synthetic_socio_wind_tunnel/memory/__init__.py` 导出
  `CarryoverContext` 与新方法

## 8. 测试

- [ ] 8.1 新建 `tests/test_multi_day.py`：
...
- [ ] 8.2 新建 `tests/test_carryover.py`：
...
- [ ] 8.5 扩展 `tests/test_memory.py`：
  - `test_daily_summary_indexed_by_day_index`
  - `test_process_tick_propagates_day_index`

## 9. Fitness-audit 扩展

- [ ] 9.1 `synthetic_socio_wind_tunnel/fitness/audits/phase2_gaps.py`：
  新增 `phase2-gaps.multi-day-run` 探针（`_module_exists` 模式），
  mitigation_change = `multi-day-simulation`
- [ ] 9.2 跑 `make fitness-audit`，确认该条从 fail → pass

## 10. 文档与示例

- [ ] 10.1 新建 `docs/agent_system/14-multi-day-simulation.md`：
...
- [ ] 10.2 更新 `README.md` 的 Development Status 表：为 `orchestrator` 行
  补一条"多日扩展（multi-day-simulation）"状态
- [ ] 10.3 更新 `docs/agent_system/08-orchestrator-tick-loop.md`：加一个
  "多日调用" 附录章节

## 11. 性能 & 回归

- [ ] 11.1 运行全部 `pytest tests/`，确认单日路径 0 回归
- [ ] 11.2 运行 `tools/smoke_experiment_demo.py --agents 100`（不带
  --multi-day）：8/8 PASS 输出不变
- [ ] 11.3 运行 `tools/smoke_experiment_demo.py --agents 100 --multi-day`
  （新开关）：确认多日路径可跑通，wall time 报告记入 demo 输出
- [ ] 11.4 跑一次 `tools/run_multi_day_experiment.py --num-days 14
  --agents 100 --seeds 5 --mode publishable --variant baseline`，
  确认 wall time ≤ 30s/seed、总 ≤ 3 分钟

## 12. 验证

- [ ] 12.1 `openspec validate multi-day-simulation --strict` 通过
- [ ] 12.2 grep 检查：`MultiDayRunner` / `CarryoverContext` / `day_index`
  在 spec / 代码 / 测试三处一致
- [ ] 12.3 确认所有 MODIFIED Requirement 的 Scenario 新增条目对应代码有
  测试覆盖
