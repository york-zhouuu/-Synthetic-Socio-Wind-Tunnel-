...
- [x] 1.1 新建 `tools/suite_stub_llm.py`：
...
- [x] 1.2 输出 JSON 格式与 Planner.replan 的 parse 一致（含 time /
  action / destination / activity / duration_minutes / reason /
  social_intent 七字段）

## 2. run_variant_suite.py CLI 整合

- [x] 2.1 import MemoryService / Planner / StubReplanLLM
- [x] 2.2 `run_seed_with_metrics` 构造：
...
- [x] 2.3 累加 `memory.process_tick` 返回的 replan list：
...
- [x] 3.1 `tools/suite_stub_llm.py` 加 `_AnthropicClient`（或同目录 helper）：
...
- [x] 4.1 `tests/test_suite_stub_llm.py`：
...
- [x] 4.2 `tests/test_suite_wiring.py`：
...
- [x] 5.1 新建 `docs/agent_system/17-suite-wiring.md`：
...
- [x] 5.2 更新 `README.md` Development Status：加 "Suite wiring
  (variant→replan 因果链)" ✅
- [x] 5.3 更新 `docs/agent_system/16-metrics.md` 的 "已知限制" 段：
  - 移除 "variant 不影响 agent 行为" 的讨论（现已修）
  - 加一行指向 `17-suite-wiring.md`

## 6. 回归 + smoke

- [x] 6.1 全 pytest 回归：零 Phase 1 / Phase 2 / metrics / policy-hack 失败
- [x] 6.2 6-variant × 2 seed × 3 day smoke：
...
- [x] 7.1 14 day × 100 agent × 1 seed wall time ≤ 35s
  （metrics-only 12s + memory.process_tick × 14 day × 288 tick × 100 agent
  期望 < 15s 额外；总 < 35s）

## 8. 验证

- [x] 8.1 `openspec validate suite-wiring --strict` 通过
- [x] 8.2 grep 检查 `synthetic_socio_wind_tunnel/` 无任何 `.py` 文件 diff
  （只改 tools/ 与 docs/ + tests/）
- [x] 8.3 grep 检查 `openspec/specs/` 下已归档 spec 文件无 diff
