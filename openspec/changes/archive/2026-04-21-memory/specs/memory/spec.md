...
agents: Mapping[str, AgentRuntime], planner: Planner | None) -> None`：
...
`attach_to(orchestrator)` SHALL 注册 `process_tick` 到 orchestrator 的
`on_tick_end` hook，避免用户手工 wire。
...
- 回填到对应 MemoryEvent（替换 event 的不可变副本）。
...
`synthetic_socio_wind_tunnel.memory` 模块在本 change 完成后 SHALL
importable；因此 `phase2-gaps.memory` AuditResult SHALL 自动由 FAIL 转
PASS（经 `_module_exists` 探针）。

#### Scenario: 审计自动翻绿
- **WHEN** 本 change archive 后运行 `make fitness-audit`
