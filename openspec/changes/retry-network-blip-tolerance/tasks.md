...
- [ ] 1.1 新建 `tests/test_retry_policy_class_name_classify.py`：
...
- [ ] 1.2 跑 → 红（`retryable_exc_class_names` 字段不存在或默认空 →
  AttributeError / 全部返回 unknown）

## 2. TDD 红 — _run_with_retry integration tests

- [ ] 2.1 新建 `tests/test_run_with_retry_class_name_path.py`：
...
- [ ] 3.1 新建 `tests/test_retry_policy_env_class_names.py`：
...
- [ ] 4.1 新建 `tests/test_retry_policy_class_name_property.py`：
